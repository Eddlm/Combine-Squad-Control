...
function ENT:Initialize()
	if SERVER then
		...
		self:SetName("Combine Mine")
		self:SetNWString("name","Combine Mine")
	...
	if SERVER then
		...
		if self.armed then
			...
			--	local t = pk_pills.getAiTeam(e)
				if 1==1 then
					if (table.HasValue(AllCombineEntities, e:GetClass()) and e != self) or (e:IsPlayer() and e:GetNWString("side") == "combine") then
						goGreen=true
					elseif e:IsNPC() or e:IsPlayer() then
						goRed=true
						if self:GetPos():Distance(e:GetPos())<150 and self.critical==false then
							...
							timer.Simple(0.01,function()
								self:GetPhysicsObject():ApplyForceCenter(Vector(0,0,3000)+(e:GetVelocity()*10 ))
